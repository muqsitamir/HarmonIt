...
\begin{table*}[t]
\centering
\caption{Source-subject results ($n=90$). PSNR and XCorr compare output with input; $\Delta W_{\mathrm{NYU}}$ is the change in Wasserstein-1 distance to the NYU reference (negative is closer). Site BA: frozen probe with 95\% subject-bootstrap interval; retrained site probes and slice probes trained on raw slices or on the method's own outputs, as mean [min, max] over seeds. Raw-input BA is on unharmonized images; chance is 0.06. $^\dagger$Fit on the test cohort. $^\ddagger$Pooled 17-site reference.}
\label{tab:main}
\footnotesize
...
\caption{(a) Site BA versus change from the input for each output. Blue: frozen probe with 95\% interval; orange: retrained probes (mean and range over seeds), joined to the frozen estimate. (b) Slice probes trained on raw slices or on each method's outputs, evaluated on that method's test outputs; one marker per seed, bars show the mean.}
\label{fig:verdicts}
\end{figure}

\section{Results}
\label{sec:results}
\textbf{Probe validity.} On unharmonized source images the frozen probe reached BA 0.95 [0.90, 1.00] and the seed-42 retrained probe 0.81 [0.75, 0.87] \pending{seeds 1--4}. The shuffled-label control scored 0.03 [0.01, 0.05], at chance, so the probes recognize sites rather than exploiting label leakage.
