"""Histogram-matching baseline for the ABIDE fixed-slice benchmark.

This script learns a foreground intensity reference distribution from the
training split and applies per-slice quantile matching to a deterministic
evaluation split. It is intentionally simple: the method only changes image
intensities inside the foreground and keeps background pixels at zero.

The saved NPZ follows the same artifact contract as ``neurocombat.py`` so it
can be evaluated by ``scripts/eval_harmonized_npz.py``.
"""

from __future__ import annotations

import argparse
import json
import os
import random
import sys
from pathlib import Path
from typing import Any, Iterable

import numpy as np
import pandas as pd


REPO_ROOT = Path(__file__).resolve().parents[2]
SRC_ROOT = REPO_ROOT / "src"
if str(SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(SRC_ROOT))


METHOD = "histogram_matching"


def parse_out_hw(values: Iterable[int]) -> tuple[int, int]:
    values = tuple(int(v) for v in values)
    if len(values) == 1:
        return values[0], values[0]
    if len(values) == 2:
        return values[0], values[1]
    raise argparse.ArgumentTypeError("--out-hw expects one integer or two integers")


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Apply train-reference foreground histogram matching to ABIDE fixed slices."
    )
    parser.add_argument("--manifest-path", default="data/abide_manifest.csv")
    parser.add_argument("--splits-path", default="data/splits.json")
    parser.add_argument("--reference-split", default="train", choices=("train", "val", "test"))
    parser.add_argument("--split", default="test", choices=("train", "val", "test"))
    parser.add_argument("--out-dir", default="outputs/harmonized/histogram_matching")
    parser.add_argument("--out-hw", nargs="+", type=int, default=(256, 256))
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--num-quantiles", type=int, default=101)
    parser.add_argument(
        "--max-reference-subjects",
        type=int,
        default=0,
        help=(
            "Optional deterministic stratified cap for the reference split. "
            "Use 0 to keep the full reference split."
        ),
    )
    parser.add_argument(
        "--foreground-thr",
        type=float,
        default=float(os.getenv("HIST_FG_THR", "0.02")),
        help="Only pixels above this normalized intensity are histogram-matched.",
    )
    parser.add_argument(
        "--valid-nonzero-frac",
        type=float,
        default=float(os.getenv("VALID_FG_FRAC", "0.02")),
        help="Matches the site-probe evaluation default slice foreground filter.",
    )
    parser.add_argument(
        "--fg-bbox-thr",
        type=float,
        default=float(os.getenv("FG_BBOX_THR", "0.02")),
        help="Matches the site-probe evaluation default foreground threshold.",
    )
    parser.add_argument(
        "--volume-cache-size",
        type=int,
        default=12,
        help="Number of normalized NIfTI volumes to keep in the dataset LRU cache.",
    )
    parser.add_argument(
        "--no-qc",
        action="store_true",
        help="Skip writing qc_raw_vs_histogram_matching.png.",
    )
    return parser


def set_seed(seed: int) -> None:
    try:
        import torch
    except ImportError as exc:
        raise ImportError(
            "Missing dependency 'torch'. Install the project runtime dependencies "
            "before running the histogram-matching baseline."
        ) from exc

    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)


def describe_array(name: str, array: np.ndarray) -> None:
    print(
        f"{name}: shape={array.shape} "
        f"min={float(array.min()):.6f} max={float(array.max()):.6f} "
        f"mean={float(array.mean()):.6f} std={float(array.std()):.6f}"
    )


def build_dataset(args: argparse.Namespace, split: str, out_hw: tuple[int, int]) -> Any:
    try:
        from harmonit.data.abide_slices_dataset import AbideSlicesDataset
    except ImportError as exc:
        raise ImportError(
            "Could not import harmonit.data.abide_slices_dataset. Run from the "
            "repository root with project dependencies installed."
        ) from exc

    dataset = AbideSlicesDataset(
        manifest_path=args.manifest_path,
        splits_path=args.splits_path,
        split=split,
        out_hw=out_hw,
        slice_mode="fixed",
        valid_nonzero_frac=args.valid_nonzero_frac,
        fg_bbox_thr=args.fg_bbox_thr,
        seed=args.seed,
        volume_cache_size=args.volume_cache_size,
        mask_mode="none",
        bg_suppress=True,
        input_mode="image",
    )
    dataset.aug_affine = False
    return dataset


def cap_dataset_samples(dataset: Any, max_samples: int, seed: int) -> None:
    if max_samples <= 0 or len(dataset.samples) <= max_samples:
        return

    rng = np.random.RandomState(seed)
    by_site: dict[int, list[Any]] = {}
    for sample in dataset.samples:
        by_site.setdefault(int(sample.site_id), []).append(sample)

    for samples in by_site.values():
        samples.sort(key=lambda sample: str(sample.subject_id))
        rng.shuffle(samples)

    selected = []
    site_ids = sorted(by_site)
    while len(selected) < max_samples:
        added = False
        for site_id in site_ids:
            samples = by_site[site_id]
            if samples:
                selected.append(samples.pop(0))
                added = True
                if len(selected) >= max_samples:
                    break
        if not added:
            break

    selected.sort(key=lambda sample: (str(sample.site), str(sample.subject_id)))
    dataset.samples = selected
    dataset._valid_slices.clear()
    dataset._vol_cache.clear()


def extract_fixed_slices(
    dataset: Any,
    batch_size: int,
    num_workers: int,
    out_hw: tuple[int, int],
    desc: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    from torch.utils.data import DataLoader
    from tqdm import tqdm

    if len(dataset) == 0:
        raise ValueError("ABIDE split is empty; no fixed slices to load")

    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=False,
        persistent_workers=(num_workers > 0),
    )

    image_batches: list[np.ndarray] = []
    subject_ids: list[str] = []
    site_ids: list[int] = []
    slice_indices: list[int] = []

    for images, sites, subjects, slices in tqdm(loader, desc=desc):
        expected_shape = (images.shape[0], 1, out_hw[0], out_hw[1])
        if tuple(images.shape) != expected_shape:
            raise AssertionError(
                f"Expected batch image shape {expected_shape}, got {tuple(images.shape)}"
            )

        image_batches.append(images.cpu().numpy().astype(np.float32, copy=False))
        subject_ids.extend(str(subject_id) for subject_id in subjects)
        site_ids.extend(int(site_id) for site_id in sites.cpu().numpy().tolist())
        slice_indices.extend(int(slice_idx) for slice_idx in slices.cpu().numpy().tolist())

    raw_images = np.concatenate(image_batches, axis=0)
    if raw_images.shape[0] != len(dataset):
        raise AssertionError(
            f"Loaded {raw_images.shape[0]} images, but dataset has {len(dataset)} samples"
        )
    if raw_images.shape[1:] != (1, out_hw[0], out_hw[1]):
        raise AssertionError(
            f"Expected per-sample image shape {(1, out_hw[0], out_hw[1])}, "
            f"got {raw_images.shape[1:]}"
        )
    if not np.isfinite(raw_images).all():
        raise AssertionError("Fixed slices contain NaN or inf values")

    return (
        raw_images,
        np.asarray(subject_ids, dtype=str),
        np.asarray(site_ids, dtype=np.int64),
        np.asarray(slice_indices, dtype=np.int64),
    )


def compute_reference_quantiles(
    reference_images: np.ndarray,
    foreground_thr: float,
    num_quantiles: int,
) -> tuple[np.ndarray, np.ndarray]:
    if num_quantiles < 3:
        raise ValueError("--num-quantiles must be >= 3")

    foreground = reference_images[reference_images > foreground_thr].astype(np.float64)
    if foreground.size == 0:
        raise ValueError(
            f"No reference foreground pixels found above foreground_thr={foreground_thr}"
        )

    quantile_grid = np.linspace(0.0, 100.0, num_quantiles, dtype=np.float64)
    reference_values = np.percentile(foreground, quantile_grid).astype(np.float64)
    reference_values = np.maximum.accumulate(reference_values)
    return quantile_grid, reference_values


def match_slice_to_reference(
    image: np.ndarray,
    reference_values: np.ndarray,
    quantile_grid: np.ndarray,
    foreground_thr: float,
) -> np.ndarray:
    matched = image.astype(np.float32, copy=True)
    mask = image > foreground_thr
    if not mask.any():
        return matched

    source_values = np.percentile(image[mask], quantile_grid).astype(np.float64)
    source_values = np.maximum.accumulate(source_values)
    unique_source, unique_indices = np.unique(source_values, return_index=True)
    unique_reference = reference_values[unique_indices]

    if unique_source.size < 2:
        matched[mask] = np.float32(np.median(reference_values))
        return matched

    transformed = np.interp(
        image[mask].astype(np.float64),
        unique_source,
        unique_reference,
        left=unique_reference[0],
        right=unique_reference[-1],
    )
    matched[mask] = transformed.astype(np.float32)
    matched[~mask] = 0.0
    return np.clip(matched, 0.0, 1.0)


def apply_histogram_matching(
    raw_images: np.ndarray,
    reference_values: np.ndarray,
    quantile_grid: np.ndarray,
    foreground_thr: float,
) -> np.ndarray:
    n_images, channels, height, width = raw_images.shape
    if channels != 1:
        raise ValueError(f"Expected single-channel slices, got C={channels}")

    harmonized = np.empty_like(raw_images, dtype=np.float32)
    for idx in range(n_images):
        harmonized[idx, 0] = match_slice_to_reference(
            raw_images[idx, 0],
            reference_values=reference_values,
            quantile_grid=quantile_grid,
            foreground_thr=foreground_thr,
        )

    if harmonized.shape != (n_images, 1, height, width):
        raise AssertionError(f"Unexpected harmonized shape: {harmonized.shape}")
    if not np.isfinite(harmonized).all():
        raise AssertionError("Histogram-matching output contains NaN or inf values")
    return harmonized


def save_manifest(
    path: Path,
    subject_ids: np.ndarray,
    site_ids: np.ndarray,
    slice_indices: np.ndarray,
    split: str,
) -> None:
    manifest = pd.DataFrame(
        {
            "row_idx": np.arange(len(subject_ids), dtype=np.int64),
            "subject_id": subject_ids,
            "site_id": site_ids,
            "slice_idx": slice_indices,
            "method": METHOD,
            "split": split,
        }
    )
    manifest.to_csv(path, index=False)


def save_qc_grid(
    raw_images: np.ndarray,
    harmonized_images: np.ndarray,
    subject_ids: np.ndarray,
    site_ids: np.ndarray,
    path: Path,
    max_subjects: int = 6,
) -> None:
    try:
        import matplotlib

        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    except Exception as exc:  # pragma: no cover - optional debug artifact
        print(f"Skipping QC image because matplotlib could not be imported: {exc}")
        return

    n = min(max_subjects, raw_images.shape[0])
    if n == 0:
        return

    fig, axes = plt.subplots(2, n, figsize=(2.4 * n, 4.8), squeeze=False)
    for col in range(n):
        raw = raw_images[col, 0]
        harmonized = harmonized_images[col, 0]
        vmin = float(min(raw.min(), harmonized.min()))
        vmax = float(max(raw.max(), harmonized.max()))

        axes[0, col].imshow(raw, cmap="gray", vmin=vmin, vmax=vmax)
        axes[0, col].set_title(f"{subject_ids[col]}\nsite {site_ids[col]}", fontsize=8)
        axes[0, col].axis("off")

        axes[1, col].imshow(harmonized, cmap="gray", vmin=vmin, vmax=vmax)
        axes[1, col].axis("off")

    axes[0, 0].set_ylabel("raw", fontsize=10)
    axes[1, 0].set_ylabel(METHOD, fontsize=10)
    fig.tight_layout()
    fig.savefig(path, dpi=160)
    plt.close(fig)


def main() -> None:
    parser = build_arg_parser()
    args = parser.parse_args()
    try:
        out_hw = parse_out_hw(args.out_hw)
    except argparse.ArgumentTypeError as exc:
        parser.error(str(exc))

    set_seed(args.seed)

    output_dir = Path(args.out_dir) / args.split
    output_dir.mkdir(parents=True, exist_ok=True)

    reference_dataset = build_dataset(args, split=args.reference_split, out_hw=out_hw)
    cap_dataset_samples(
        reference_dataset,
        max_samples=args.max_reference_subjects,
        seed=args.seed,
    )
    reference_images, _, reference_site_ids, _ = extract_fixed_slices(
        dataset=reference_dataset,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        out_hw=out_hw,
        desc=f"Loading {args.reference_split} reference fixed ABIDE slices",
    )
    quantile_grid, reference_values = compute_reference_quantiles(
        reference_images=reference_images,
        foreground_thr=args.foreground_thr,
        num_quantiles=args.num_quantiles,
    )

    dataset = build_dataset(args, split=args.split, out_hw=out_hw)
    raw_images, subject_ids, site_ids, slice_indices = extract_fixed_slices(
        dataset=dataset,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        out_hw=out_hw,
        desc=f"Loading {args.split} fixed ABIDE slices",
    )

    print(f"Reference split: {args.reference_split}")
    print(f"Reference subjects: {reference_images.shape[0]}")
    print(
        f"Reference unique sites: {len(np.unique(reference_site_ids))} "
        f"-> {np.unique(reference_site_ids).tolist()}"
    )
    print(f"Split: {args.split}")
    print(f"Subjects: {raw_images.shape[0]}")
    print(f"Unique sites: {len(np.unique(site_ids))} -> {np.unique(site_ids).tolist()}")
    print("First deterministic rows:")
    for row in range(min(5, len(subject_ids))):
        print(
            f"  {row}: subject_id={subject_ids[row]} "
            f"site_id={int(site_ids[row])} slice_idx={int(slice_indices[row])}"
        )
    describe_array("Before histogram matching", raw_images)

    harmonized_images = apply_histogram_matching(
        raw_images=raw_images,
        reference_values=reference_values,
        quantile_grid=quantile_grid,
        foreground_thr=args.foreground_thr,
    )

    describe_array("After histogram matching", harmonized_images)

    npz_path = output_dir / "histogram_matching_slices.npz"
    np.savez_compressed(
        npz_path,
        images=harmonized_images,
        raw_images=raw_images,
        subject_ids=subject_ids,
        site_ids=site_ids,
        slice_indices=slice_indices,
        split=np.asarray(args.split),
        method=np.asarray(METHOD),
    )

    manifest_path = output_dir / "manifest.csv"
    save_manifest(
        manifest_path,
        subject_ids=subject_ids,
        site_ids=site_ids,
        slice_indices=slice_indices,
        split=args.split,
    )

    if not args.no_qc:
        save_qc_grid(
            raw_images=raw_images,
            harmonized_images=harmonized_images,
            subject_ids=subject_ids,
            site_ids=site_ids,
            path=output_dir / "qc_raw_vs_histogram_matching.png",
        )

    config = {
        "method": METHOD,
        "split": args.split,
        "reference_split": args.reference_split,
        "manifest_path": args.manifest_path,
        "splits_path": args.splits_path,
        "out_hw": list(out_hw),
        "slice_mode": "fixed",
        "input_mode": "image",
        "mask_mode": "none",
        "bg_suppress": True,
        "valid_nonzero_frac": args.valid_nonzero_frac,
        "fg_bbox_thr": args.fg_bbox_thr,
        "foreground_thr": args.foreground_thr,
        "num_quantiles": args.num_quantiles,
        "max_reference_subjects": args.max_reference_subjects,
        "seed": args.seed,
        "n_reference_subjects": int(reference_images.shape[0]),
        "n_reference_sites": int(len(np.unique(reference_site_ids))),
        "n_subjects": int(raw_images.shape[0]),
        "n_sites": int(len(np.unique(site_ids))),
        "reference_quantiles": {
            "grid": quantile_grid.tolist(),
            "values": reference_values.tolist(),
        },
    }
    (output_dir / "config.json").write_text(json.dumps(config, indent=2) + "\n")

    print(f"Saved: {npz_path}")
    print(f"Saved: {manifest_path}")


if __name__ == "__main__":
    main()
