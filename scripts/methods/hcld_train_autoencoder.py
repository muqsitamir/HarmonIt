...
def build_models(args: argparse.Namespace, device: torch.device) -> tuple[torch.nn.Module, torch.nn.Module]:
    ...
    autoencoder = AutoencoderKL(
        ...
        norm_num_groups=8,
        attention_levels=tuple(i == len(channels) - 1 for i in range(len(channels))),
...
@torch.no_grad()
def validate(model: torch.nn.Module, loader: DataLoader, device: torch.device, max_batches: int = 8) -> float:
    model.eval()
    losses = []
    ...
    for epoch in range(start_epoch, args.epochs):
        for batch in train_loader:
            ...
            if step % args.val_every_steps == 0:
                ...
                print(f"validation step={step} val_l1={val_l1:.6f}", flush=True)
